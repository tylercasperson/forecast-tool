...
export const createBulkSalesData = (salesData) => async (dispatch) => {
  try {
    dispatch({ type: SALES_DATA_BULK_CREATE_REQUEST });

    const { data } = await axios.post(`/api/salesData/bulk/add`, salesData);

    dispatch({
      type: SALES_DATA_BULK_CREATE_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
